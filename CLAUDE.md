...
- **`README.md`'s "Security & Compliance" section (lines 624–682) overstates the implementation.** It claims AES-256 encryption at rest, column-level encryption for embeddings, and row-level security as enforced runtime protection. None of those exist in application code: encryption at rest is whatever Neon/Supabase do by default, there is no field-level encryption, and while `supabase/migrations/0005_enable_rls_public_tables.sql` does enable RLS, the app connects as table owner and bypasses it (it guards the PostgREST path only — app-layer `organizationId` scoping is the real isolation). Do not source security or marketing copy from that section. `client/src/pages/privacy.tsx:69` also tells the public that content is processed by "OpenAI and Anthropic" — the app is OpenAI-only. Both unfixed. A code-verified inventory of what we *can* claim is in `docs/security-page-plan.md`.
