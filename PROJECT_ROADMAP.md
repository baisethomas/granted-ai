...
Security hardening (out of phase, but ahead of the Phase 6 security pass): a cross-tenant data leak was found and fixed in **PR #49**. RLS was disabled on all 19 public tables while the Supabase Data API was on, so the browser anon key could read every row via PostgREST. Fix: RLS enabled on all tables (`supabase/migrations/0005_enable_rls_public_tables.sql`) and the Data API disabled in the dashboard. This does not complete Phase 6's security pass — it closes one specific critical finding. Open follow-up: remove the dormant, UI-unreachable Passport-local auth routes (`server/auth.ts`).
