...
export class FileProcessor {
  async processFile(
    ...
    try {
      ...
      extractedText = `[Error extracting content from ${filename}: ${
        error instanceof Error ? error.message : "Unknown error"
      }]`;
    }

    const rawText = extractedText.substring(0, 500_000);
    const rawTextBytes = Buffer.byteLength(rawText, "utf-8");
    ...
    const summary = await aiService.summarizeDocument(rawText, filename);
